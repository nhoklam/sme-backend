...
public class PurchaseService {
    ...
    public PurchaseOrder approvePurchaseOrder(UUID poId, UUID approvedBy) {
        ...
        if (po.getWarehouseId() == null) {
            throw new BusinessException("MISSING_WAREHOUSE",
                    "Phiếu nhập kho này chưa được gán chi nhánh. Vui lòng kiểm tra lại.");
        }
        ...
        try {
            ...
            if (po.getItems() != null) {
                for (PurchaseItem item : po.getItems()) {
                    ...
                    inventoryService.importStock(item.getProductId(), po.getWarehouseId(), quantity, importPrice,
                            po.getId(), operator);
                }
            }
            ...
            int paymentTerms = (supplier != null && supplier.getPaymentTerms() != null) ? supplier.getPaymentTerms()
                    : 30;
            ...
            log.error("Business error during PO approval: {}", be.getMessage());
            throw be;
        } catch (Exception e) {
            log.error("Critical error during PO approval: ", e);
            throw new BusinessException("APPROVE_FAILED", "Duyệt phiếu thất bại: " + e.getMessage());
        }
    }
    ...
    public PurchaseOrder cancelPurchaseOrder(UUID poId, String reason) {
        ...
        if (po.getStatus() == PurchaseOrder.PurchaseStatus.COMPLETED) {
            throw new BusinessException("CANNOT_CANCEL", "Không thể hủy phiếu đã hoàn thành");
        }
    ...
    public Page<PurchaseOrder> searchOrders(UUID warehouseId, String keyword, PurchaseOrder.PurchaseStatus status,
            Pageable pageable) {
